...
* 「:ref:`D102480C-FFFB-43BA-9561-291E1AF4255B`」を追加
...
* 非表示のモデルに対して物理演算を発生させないようにした
  ...
  * MMD の場合は非表示のモデルでも物理演算の衝突判定が発生するのでご注意ください
...
* プロジェクト保存時にモデルが未選択の場合読み込み時に反映されない
* Windows 版でエラー表示において文字化けが発生することがある
* ``Post_ScreenTex.fx`` を利用すると描画が正しく行われない
* カメラのアングルが間違っている

  * 32.0 以前で保存されたプロジェクトについては読み込み時に補正します
  * カメラモーションとして書き出してそれを読み込んだ場合は補正しないため再度書き出してください
