...
BaseApplicationService::dispatchMenuItemAction(Project *project, nanoem_u32_t type, Error &error)
{
    ...
    case ApplicationMenuBuilder::kMenuItemTypeFileNewModel: {
        static const nanoem_u8_t kNewModelInJapanese[] = { 0xe6, 0x96, 0xb0, 0xe8, 0xa6, 0x8f, 0xe3, 0x83, 0xa2, 0xe3,
            0x83, 0x87, 0xe3, 0x83, 0xab, 0 };
        const UUID uuid(project->generateUUID(this));
        ...
        StringUtils::format(desc.m_nameInJapanese, "%s-%s", kNewModelInJapanese, shortUUID);
        StringUtils::format(desc.m_nameInEnglish, "NewModel-%s", shortUUID);
        StringUtils::format(desc.m_commentInJapanese, "This model was generated by nanoem\n\nUUID: %s\nDateTime: %s",
            uuidString.c_str(), dateTimeBuffer);
        desc.m_commentInEnglish = desc.m_commentInJapanese;
        ...
        if (status == NANOEM_STATUS_SUCCESS) {
            ...
            String newModelPath(project->fileURI().absolutePathByDeletingLastPathComponent());
            newModelPath.append("/");
            const char *filename = project->castLanguage() == NANOEM_LANGUAGE_TYPE_JAPANESE
                ? desc.m_nameInJapanese.c_str()
                : desc.m_nameInEnglish.c_str();
