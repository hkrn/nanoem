 ...
 *  Direct3D shaders.
 *
 * Please see the file LICENSE.md in the source's root directory.
 *
 ...
 *  4-nanoem_f32_t vector to be specified for what would be register "c6" in D3D
 ...
 *  type==MOJOSHADER_UNIFORM_FLOAT, that means we'd expect a 4-nanoem_f32_t vector
...
typedef struct MOJOSHADER_constant
{
    ...
    union
    {
        nanoem_f32_t f[4];  /* if type==MOJOSHADER_UNIFORM_FLOAT */
...
typedef union MOJOSHADER_astNode
{
    ...
    MOJOSHADER_astExpressionFloatLiteral nanoem_f32_tliteral;
...
/*
 * Set a nanoem_f32_ting-point uniform value (what Direct3D calls a "constant").
 *
 * There is a single array of 4-nanoem_f32_t "registers" shared by all vertex shaders.
 ...
 *
 * (idx) is the index into the internal array: 0 is the first four nanoem_f32_ts,
 *  1 is the next four, etc.
 * (data) is a pointer to (vec4count*4) nanoem_f32_ts.
 *
 * This call is NOT thread safe! As most OpenGL implementations are not thread
 *  safe, you should probably only call this from the same thread that created
 *  the GL context.
 ...
 */
DECLSPEC void MOJOSHADER_glSetVertexShaderUniformF(unsigned int idx, const nanoem_f32_t *data,
                                                   unsigned int vec4count);

/*
 * Retrieve a nanoem_f32_ting-point uniform value (what Direct3D calls a "constant").
 *
 * There is a single array of 4-nanoem_f32_t "registers" shared by all vertex shaders.
 *  This is the "c" register file in Direct3D (c0, c1, c2, etc...)
 *  MojoShader will take care of synchronizing this internal array with the
 ...
 * (idx) is the index into the internal array: 0 is the first four nanoem_f32_ts,
 *  1 is the next four, etc.
 * (data) is a pointer to space for (vec4count*4) nanoem_f32_ts.
 ...
 * Uniforms are not shared between contexts.
 */
DECLSPEC void MOJOSHADER_glGetVertexShaderUniformF(unsigned int idx, nanoem_f32_t *data,
                                                   unsigned int vec4count);
 ...
 *  appropriate variables in the GL shaders.
 *
 * Unlike the nanoem_f32_t and int counterparts, booleans are single values, not
 ...
 * Uniforms are not shared between contexts.
 */
DECLSPEC void MOJOSHADER_glSetPixelShaderUniformF(unsigned int idx, const nanoem_f32_t *data,
                                                  unsigned int vec4count);
 ...
 * Uniforms are not shared between contexts.
 */
DECLSPEC void MOJOSHADER_glGetPixelShaderUniformF(unsigned int idx, nanoem_f32_t *data,
                                                  unsigned int vec4count);
 ...
 */
DECLSPEC void MOJOSHADER_glSetLegacyBumpMapEnv(unsigned int sampler, nanoem_f32_t mat00,
                                               nanoem_f32_t mat01, nanoem_f32_t mat10, nanoem_f32_t mat11,
                                               nanoem_f32_t lscale, nanoem_f32_t loffset);
