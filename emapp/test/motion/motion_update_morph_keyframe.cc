...
TEST_CASE("motion_update_morph_keyframe", "[emapp][motion]")
{
    TestScope scope;
    {
        ...
        {
            model::Morph *morph = model::Morph::cast(activeModel->activeMorph());
            morph->setWeight(0.84f);
        }
        registrator.registerAddMorphKeyframesCommandByAllMorphs(activeModel);
        const nanoem_unicode_string_t *name =
            nanoemModelMorphGetName(activeModel->activeMorph(), NANOEM_LANGUAGE_TYPE_FIRST_ENUM);
        SECTION("updating current active model morph keyframe")
        {
            ...
            CHECK(nanoemMotionMorphKeyframeGetWeight(keyframe) == Approx(0.84f));
