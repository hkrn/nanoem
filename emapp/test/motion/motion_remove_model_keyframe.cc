...
TEST_CASE("motion_remove_model_keyframe", "[emapp][motion]")
{
    TestScope scope;
    {
        ...
        registrator.registerRemoveModelKeyframesCommandByCurrentLocalFrameIndex(activeModel);
        SECTION("removing model keyframe")
        {
            CHECK(first->countAllModelKeyframes(activeModel) == 1);
            CHECK(first->countAllModelKeyframes(other1Model) == 1);
            CHECK(first->countAllModelKeyframes(other2Model) == 1);
            CHECK_FALSE(nanoemMotionFindModelKeyframeObject(project->resolveMotion(activeModel)->data(), 1337));
        ...
        SECTION("recover")
        {
            ...
            CHECK(second->countAllModelKeyframes(activeModel2) == 1);
            CHECK_FALSE(nanoemMotionFindModelKeyframeObject(project2->resolveMotion(activeModel2)->data(), 1337));
