...
TEST_CASE("motion_remove_bone_keyframe_at_zero", "[emapp][motion]")
{
    TestScope scope;
    {
        ...
        SECTION("removing active bone keyframe")
        {
            ...
            CHECK(glm::make_vec4(nanoemMotionBoneKeyframeGetInterpolation(keyframe,
                      NANOEM_MOTION_BONE_KEYFRAME_INTERPOLATION_TYPE_TRANSLATION_X)) == glm::u8vec4(20, 20, 107, 107));
            CHECK(glm::make_vec4(nanoemMotionBoneKeyframeGetInterpolation(keyframe,
                      NANOEM_MOTION_BONE_KEYFRAME_INTERPOLATION_TYPE_TRANSLATION_Y)) == glm::u8vec4(20, 20, 107, 107));
            CHECK(glm::make_vec4(nanoemMotionBoneKeyframeGetInterpolation(keyframe,
                      NANOEM_MOTION_BONE_KEYFRAME_INTERPOLATION_TYPE_TRANSLATION_Z)) == glm::u8vec4(20, 20, 107, 107));
            CHECK(glm::make_vec4(nanoemMotionBoneKeyframeGetInterpolation(keyframe,
                      NANOEM_MOTION_BONE_KEYFRAME_INTERPOLATION_TYPE_ORIENTATION)) == glm::u8vec4(20, 20, 107, 107));
            CHECK(keyframe);
            CHECK(project->duration() == project->baseDuration());
            CHECK(first->motionDuration(activeModel) == 0);
            CHECK(first->motionDuration(other1Model) == 0);
