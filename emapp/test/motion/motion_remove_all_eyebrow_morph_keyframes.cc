...
TEST_CASE("motion_remove_all_eyebrow_morph_keyframes", "[emapp][motion]")
{
    TestScope scope;
    {
        ...
        CommandRegistrator registrator(project);
        {
            ...
            const nanoem_model_morph_t *morph = first->findFirstMorph(activeModel, NANOEM_MODEL_MORPH_CATEGORY_EYEBROW);
        ...
        registrator.registerRemoveAllEyebrowMorphKeyframesCommand(activeModel);
        SECTION("remove all eyebrow morph keyframes")
        {
            CHECK(first->motionDuration(activeModel) == 20);
            CHECK(first->countAllMorphKeyframes(activeModel) == 150);
            CHECK(first->countAllMorphKeyframesByCategory(activeModel, NANOEM_MODEL_MORPH_CATEGORY_EYE) == 66);
            CHECK(first->countAllMorphKeyframesByCategory(activeModel, NANOEM_MODEL_MORPH_CATEGORY_EYEBROW) == 6);
            ...
            CHECK(
                first->findFirstMorphKeyframeWeight(activeModel, NANOEM_MODEL_MORPH_CATEGORY_EYEBROW) == Approx(0.0f));
        ...
        SECTION("undo")
        {
            ...
            CHECK(first->countAllMorphKeyframesByCategory(activeModel, NANOEM_MODEL_MORPH_CATEGORY_OTHER) == 12);
            CHECK(
                first->findFirstMorphKeyframeWeight(activeModel, NANOEM_MODEL_MORPH_CATEGORY_EYEBROW) == Approx(0.8f));
            CHECK_FALSE(scope.hasAnyError());
        }
        ...
        SECTION("recover")
        {
            ...
            CHECK(second->countAllMorphKeyframes(activeModel2) == 150);
            CHECK(second->countAllMorphKeyframesByCategory(activeModel2, NANOEM_MODEL_MORPH_CATEGORY_EYE) == 66);
            CHECK(second->countAllMorphKeyframesByCategory(activeModel2, NANOEM_MODEL_MORPH_CATEGORY_EYEBROW) == 6);
            CHECK(second->countAllMorphKeyframesByCategory(activeModel2, NANOEM_MODEL_MORPH_CATEGORY_LIP) == 66);
            CHECK(second->countAllMorphKeyframesByCategory(activeModel2, NANOEM_MODEL_MORPH_CATEGORY_OTHER) == 12);
            CHECK(second->findFirstMorphKeyframeWeight(activeModel2, NANOEM_MODEL_MORPH_CATEGORY_EYEBROW) ==
                Approx(0.0f));
            CHECK_FALSE(scope.hasAnyError());
        }
    }
