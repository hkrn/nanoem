...
TEST_CASE("motion_remove_morph_keyframe", "[emapp][motion]")
{
    TestScope scope;
    {
        ...
        registrator.registerRemoveMorphKeyframesCommandByCurrentLocalFrameIndex(activeModel);
        const nanoem_unicode_string_t *name =
            nanoemModelMorphGetName(activeModel->activeMorph(), NANOEM_LANGUAGE_TYPE_FIRST_ENUM);
        SECTION("removing active model morph keyframe")
        {
        ...
        SECTION("recover")
        {
            ...
            CHECK(second->countAllMorphKeyframes(activeModel2) == 30);
            CHECK(project2->duration() == project->baseDuration());
            CHECK(second->motionDuration(activeModel2) == 0);
            CHECK_FALSE(second->findMorphKeyframe(activeModel2, name, 1337));
            CHECK_FALSE(scope.hasAnyError());
        }
    }
...
TEST_CASE("motion_remove_morph_keyframe_at_zero", "[emapp][motion]")
{
    TestScope scope;
    {
        ProjectPtr first = scope.createProject();
        ...
        SECTION("removing active model morph keyframe")
        {
            ...
            CHECK(first->countAllMorphKeyframes(other1Model) == 30);
            CHECK(first->countAllMorphKeyframes(other2Model) == 30);
            const nanoem_motion_morph_keyframe_t *keyframe = first->findMorphKeyframe(activeModel, name, 0);
            CHECK(keyframe);
            CHECK_FALSE(first->findMorphKeyframe(other1Model, name, 1337));
            CHECK_FALSE(first->findMorphKeyframe(other2Model, name, 1337));
            CHECK(nanoemMotionMorphKeyframeGetWeight(keyframe) == Approx(0.0f));
            CHECK(project->duration() == project->baseDuration());
            CHECK(first->motionDuration(activeModel) == 0);
            CHECK(first->motionDuration(other1Model) == 0);
            CHECK(first->motionDuration(other2Model) == 0);
        ...
        SECTION("recover")
        {
            ...
            Model *activeModel2 = project2->activeModel();
            CHECK(second->countAllMorphKeyframes(activeModel2) == 30);
            const nanoem_motion_morph_keyframe_t *keyframe = second->findMorphKeyframe(activeModel2, name, 0);
            CHECK(keyframe);
            CHECK(nanoemMotionMorphKeyframeGetWeight(keyframe) == Approx(0.0f));
            CHECK(project2->duration() == project->baseDuration());
            CHECK(second->motionDuration(activeModel2) == 0);
