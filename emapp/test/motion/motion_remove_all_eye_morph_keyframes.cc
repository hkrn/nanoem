...
TEST_CASE("motion_remove_all_eye_morph_keyframes", "[emapp][motion]")
{
    TestScope scope;
    {
        ...
        CommandRegistrator registrator(project);
        {
            ...
            const nanoem_model_morph_t *morph = first->findFirstMorph(activeModel, NANOEM_MODEL_MORPH_CATEGORY_EYE);
        ...
        registrator.registerRemoveAllEyeMorphKeyframesCommand(activeModel);
        SECTION("removing all eye morph keyframes")
        {
            ...
            CHECK(first->countAllMorphKeyframesByCategory(activeModel, NANOEM_MODEL_MORPH_CATEGORY_EYE) == 11);
            ...
            CHECK(first->findFirstMorphKeyframeWeight(activeModel, NANOEM_MODEL_MORPH_CATEGORY_EYE) == Approx(0.0f));
        ...
        SECTION("undo")
        {
            ...
            CHECK(first->countAllMorphKeyframesByCategory(activeModel, NANOEM_MODEL_MORPH_CATEGORY_LIP) == 66);
            CHECK(first->countAllMorphKeyframesByCategory(activeModel, NANOEM_MODEL_MORPH_CATEGORY_OTHER) == 12);
            CHECK(first->findFirstMorphKeyframeWeight(activeModel, NANOEM_MODEL_MORPH_CATEGORY_EYE) == Approx(0.8f));
            CHECK_FALSE(scope.hasAnyError());
        }
        ...
        SECTION("recover")
        {
            ...
            CHECK(second->countAllMorphKeyframesByCategory(activeModel2, NANOEM_MODEL_MORPH_CATEGORY_EYE) == 11);
            ...
            CHECK(second->findFirstMorphKeyframeWeight(activeModel2, NANOEM_MODEL_MORPH_CATEGORY_EYE) == Approx(0.0f));
            CHECK_FALSE(scope.hasAnyError());
        }
    }
}
