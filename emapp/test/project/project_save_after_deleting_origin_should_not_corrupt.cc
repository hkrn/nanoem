...
TEST_CASE("project_save_after_deleting_origin_should_not_corrupt", "[emapp][project]")
{
    TestScope scope;
    Application *application = scope.application();
    {
        ...
        Error error;
        CHECK(
            application->fileManager()->saveAsFile(fileURI, IFileManager::kDialogTypeSaveProjectFile, project, error));
        ...
        CHECK(application->fileManager()->loadFromFile(
            fileURI, IFileManager::kDialogTypeOpenProject, transientProject, error));
        CHECK_FALSE(error.hasReason());
        ...
        CHECK(application->fileManager()->saveAsFile(
            fileURI, IFileManager::kDialogTypeSaveProjectFile, transientProject, error));
        CHECK_FALSE(error.hasReason());
        ProjectPtr o3 = scope.createProject();
        Project *newProject = o3.get()->m_project;
        CHECK(
