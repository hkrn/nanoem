...
TEST_CASE("accessory_set_scale_factor", "[emapp][accessory]")
{
    TestScope scope;
    {
        ...
        std::unique_ptr<AccessoryScaleFactorValueState> state(new AccessoryScaleFactorValueState(accessory));
        state->setValue(glm::value_ptr(Vector4(1.8f)));
        SECTION("commit and change scale factor")
        {
            state->commit();
            ...
            REQUIRE(accessory->scaleFactor() == Approx(1.8f));
        }
        SECTION("undo and rolled back scale factor only")
        {
            state->commit();
